...
class CrontabCoupleLuckyNumbersController extends AppController
{
	const LIMIT_DATE = 15;
	...
	*/
	public function getLuckyNumber()
	{
		...
		$arrCouple1 = $this->getNumberCouple($this->mRESULT[0]); //mảng các lô về lộn của ngày 1
		...
		if (!empty($arrCouple1)) {
			...
			foreach ($arrCouple1 as $num) {
				...
				if(!empty($arrayPosition2)){
					$arrCouple2 = $this->getNumberCouple($this->mRESULT[1]);
			...
			if( !empty($arrayPosition3) ){//nếu có cầu 2 ngày thì mới tính tiếp các cầu dài hơn
				...
				$indexLoop = 4;
				$haveResult = true;
				${"arrCouple".($indexLoop-1)} = $this->getNumberCouple($this->mRESULT[$indexLoop-2]);
			...
			debug('Ngày gần nhất khồn có loto nào về cặp.');
		}
	}
	...
	/**
    * Lấy ra tất cả các loto về lộn
    * @author: Ungnv
    * @param: mảng 27 số của bộ kết quả 1 ngày
    * @return: mảng các con lô về lộn
    */
    protected function getNumberCouple($result)
    {
        $returnArray = array();
        $lotoArray = $this->getAllLoto($result);

        for ($i=0; $i < count($lotoArray)-1; $i++) { 
            for ($j=$i+1; $j < count($lotoArray); $j++) { 
                if( $this->isCouple( $lotoArray[$i], $lotoArray[$j] ) ){
                    $returnArray[] = $lotoArray[$i];
                }
            }
        }
    ...
    /**
    * kiểm tra xem 2 loto có phải lộn của nhau không?
    * @author: Ungnv
    * @param $loto1: Lô để so sánh thứ nhất
    * @param $loto2: Lô để so sánh thứ hai
    * @return: bool
    */
    protected function isCouple($loto1, $loto2)
    {
        if( strrev($loto1) == $loto2 ){
            return true;
        }
        return false;
    }
