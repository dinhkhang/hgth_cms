...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-sm-8">
		<h2>
			...
			echo $this->Paginator->counter(
					'<small>' . __('Total record: {:count}') . '</small>'
			);
			?>
		</h2>
		<ol class="breadcrumb">
	...
	<div class="col-sm-4">
		<div class="title-action">
                ...
                if(isset($objectId, $objectTypeId)) {
                        $url = Router::url(array('action' => 'add', '?' => array('objectId' => $objectId, 'objectTypeId' => $objectTypeId)));
                } elseif(isset($objectTypeId)) {
                        $url = isset($objectId) ? Router::url(array('action' => 'add', '?' => array('objectTypeId' => $objectTypeId))) : '';
                }
                ?>
                <?php if($url) : ?>
